...
class CoursesPage extends StatelessWidget {
  CoursesPage({Key? key, String personName = 'Annastasiya Lejvchekina'})
  ...
  Widget _tileBuilder(List<String> courses) {
    ...
    for (int i = 0; i < _personCourses.length; i += 3) {
      ...
      for (int j = i; j < i + 3 && j < _personCourses.length; j++) {
        rowCourses.add(_buildCourseTile(_personCourses[j]));
        rowCourses.add(const SizedBox(width: 40));
      }
      courseRows.add(
        Row(
      ...
      body: SafeArea(
        ...
        child: ListView(
          children: [
            ...
            SizedBox(height: 120),
            FittedBox(
              child: _tileBuilder(_personCourses),
              fit: BoxFit.scaleDown,
            ),
          ],
        ),
